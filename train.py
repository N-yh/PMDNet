...
if __name__ == "__main__":
    ...
    file_psnr = 'PSNR/PSNR_PMDNet_Rain13k.txt'
    file_loss = 'LOSS/LOSS_PMDNet_Rain13k.txt'
    ...
    for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
        ...
        for i, data in enumerate(tqdm(train_loader), 0):
            ...
            # cal total loss
            loss = 0.3 * (loss_char0 - 0.5 * loss_SSIM0) + 0.06 * (loss_char1 - 0.5 * loss_SSIM1) + 0.2 * loss_edge0
        ...
        loss_values.append(epoch_loss)  # Append the current epoch's loss
        ssim_values.append(SSIM)  # Append the current epoch's SSIM
    ...
    epochs = range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1)
    ...
    plt.legend()
    plt.savefig(os.path.join(result_dir, 'ssim_plot.png')) 
